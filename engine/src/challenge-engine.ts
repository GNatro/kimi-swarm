...
function generateSummary(subject: string, reviews: LensReview[], risk: number): string {
  ...
  lines.push('');
  lines.push('## Context');
  lines.push(`This review applies the 6-Lens framework plus Red Team adversarial analysis to the proposed change: **${subject}**. Each lens examines the change from a distinct professional perspective to surface hidden risks and failure modes that a single reviewer might miss.`);
  lines.push('');
  ...
  for (const review of reviews) {
    ...
    lines.push('**Evidence required:** Cite specific files, lines, or prior incidents that support this assessment.');
    lines.push('');
  }
  
  lines.push('## Red Team Attacks');
  lines.push('');
  lines.push('The following adversarial scenarios were modeled against this change:');
  lines.push('');
  lines.push('1. **Malformed Input Injection** — Supply unexpected data shapes, nulls, or extreme values.');
  lines.push('2. **Race Condition Exploitation** — Trigger the same operation concurrently to test idempotency.');
  lines.push('3. **Resource Exhaustion** — Provide maximum-size inputs to test bounds and timeouts.');
  lines.push('4. **Privilege Escalation** — Attempt the operation with minimal or degraded privileges.');
  lines.push('5. **Dependency Compromise** — Simulate a compromised upstream dependency.');
  lines.push('');
  lines.push('For each attack, document the observed behavior and whether the system fails safely or dangerously.');
  lines.push('');
  
  lines.push('## Conclusion');
  lines.push('');
  lines.push(`This change has a risk score of ${risk}. ${risk >= 13 ? 'Recommendation: Split into smaller PRs, add dedicated security review, and require explicit sign-off from SRE and Security lenses before merge.' : 'Proceed with standard review, but ensure all lens questions are answered with evidence before approval.'}`);
  lines.push('');
  lines.push('---');
  lines.push(`*Review generated by Challenge Engine v1.0 | 6-Lens + Red Team | ${new Date().toISOString()}*`);
